...
**I've seen this play out repeatedly over the last three years building LLM-powered products at the frontier**, at [Anterior](https://www.anterior.com/insights/anterior-secures-usd20-million-series-a-to-unlock-administrative-efficiencies-for-healthcare) (automating healthcare admin for payors) and elsewhere. Some approaches stand the test of time, while others don't.

# The "Sour Lesson" of Building AI Products
**[Richard Sutton's Bitter Lesson](https://www.cs.utexas.edu/~eunsol/courses/data/bitter_lesson.pdf) points out that in machine learning, methods that simply scaling data and computation for learning and search have consistently won out in the long run** against approaches that have tried to bake-in human expertise (even if the latter *initially* performs better).
...
I view this as more art than science, and so to help illustrate what makes something 'Sour Lesson-pilled', let's review through some approaches from the last 3 years at Anterior that did and didn't survive the inexorable march of LLM progress.
...
**The Context:** In the summer of 2023, I was building our first prior authorization product using GPT-3.5-Turbo-16k. A typical prior auth case requires processing:
- Clinical guidelines (often 20-50 pages)
...
**What We Did:** I created a technique (which I referred to as 'hierarchical query reasoning') which broke policy documents into tree-structured questions where high-level policy requirements decomposed into more specific sub-questions, each answerable within the token limit.
...
I spent 1-2 months building this and it got us our first 2 enterprise customers, so it was a worthwhile time investment - but ultimately, by 6 months we had an alternative approach deployed in production.
...
**The Context:** I noticed our product was often not appreciating context-specific nuance. For example, the term "conservative therapy" can mean different things in a surgical context vs a medical context.
...
### A Framework for Sour Lesson-Pilled Decision Making
